...
const char *MISSING_PIN_LOGIC = "{\"error\":\"Faltan campos obligatorios: 'pin' y 'logica'.\"}\n";
const char *MISSING_LED_STATE = "{\"error\":\"Faltan el campo obligatorio: 'estado'.\"}\n";
...
void readLed(AsyncWebServerRequest *req) {
  if (req->pathArg(0) != "") {
    ...
    if (led_found)
      req->send(200, "application/json", led_found->toJson() + '\n');
    else
      req->send(404, "application/json", NOT_FOUND);
 ...
 */
void createLed(AsyncWebServerRequest *req, uint8_t *data, size_t len, size_t index, size_t total) {
  static String body{};
  if (!index) body = "";
 ...
 * @param total size_t tamaño total del body
 */
void updateLed(AsyncWebServerRequest *req, uint8_t *data, size_t len, size_t index, size_t total) {
  static String body{};
  if (!index) body = "";
 ...
 * PATCH  /led/{n}  : Modifica el estado del led con id "n" (formato:
...
void initServer() {
  ...
  server.on("^\\/led\\/([0-9]+)$", HTTP_PATCH, [](AsyncWebServerRequest *req) {}, NULL, updateLed);
